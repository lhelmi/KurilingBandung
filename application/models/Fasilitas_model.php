<?php

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Fasilitas_model extends CI_Model
{

    public $table = 'fasilitas';
    public $id = 'id_fasilitas';
    public $order = 'DESC';

    function __construct()
    {
        parent::__construct();
    }

    // get all
    function get_all()
    {
        $this->db->order_by($this->id, $this->order);
        return $this->db->get($this->table)->result();
    }

    // get data by id
    function get_by_id($id)
    {
        $this->db->where($this->id, $id);
        return $this->db->get($this->table)->row();
    }
    
    // get total rows
    function total_rows($q = NULL) {
        $this->db->like('id_fasilitas', $q);
	$this->db->or_like('nama_fasilitas', $q);
	$this->db->or_like('harga_fasilitas', $q);
	$this->db->or_like('id_wisata', $q);
	$this->db->from($this->table);
        return $this->db->count_all_results();
    }

    // get data with limit and search
    function get_limit_data($limit, $start = 0, $q = NULL) {
        $this->db->order_by($this->id, $this->order);
        $this->db->like('id_fasilitas', $q);
	$this->db->or_like('nama_fasilitas', $q);
	$this->db->or_like('harga_fasilitas', $q);
	$this->db->or_like('id_wisata', $q);
	$this->db->limit($limit, $start);
        return $this->db->get($this->table)->result();
    }

    // insert data
    function insert($data)
    {
        
        // $this->db->insert_batch('gambar', $data);
        $this->db->insert_batch($this->table, $data);

        // $total_array = count($data2);
         
        // if($total_array != 0){

        //     $hasil = $this->db->insert_batch('fasilitas', $data2);
        // }
    }

    // update data
    function update($data, $id_fasilitas)
    {
        //$this->db->where($this->id, $id);
        $this->db->update_batch($this->table, $data, $id_fasilitas);
    }

    // delete data
    function delete($id)
    {
        $this->db->set('is_delete', '1');
        $this->db->where($this->id, $id);
        $this->db->update($this->table, $data);
    }

}

/* End of file Fasilitas_model.php */
/* Location: ./application/models/Fasilitas_model.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2018-06-12 00:02:23 */
/* http://harviacode.com */